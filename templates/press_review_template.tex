...
# Rendu LaTeX
env = Environment(loader=FileSystemLoader(TEMPLATE_DIR), block_start_string='{%', block_end_string='%}', variable_start_string='{{', variable_end_string='}}')
...
# Ajout de sécurité : s'assurer que le document contient bien les balises LaTeX essentielles
if "\\begin{document}" not in rendered_tex or "\\end{document}" not in rendered_tex:
    print("❌ Erreur : le template LaTeX est incomplet. Vérifiez qu'il contient \\begin{document} et \\end{document}.")
else:
